...
-- pairelt takes an elt h and a list b and returns list of all pairs,
...
set_cardinality (mkSet l) = l

-------------------------------------

-- If v is in s return true, otherwise false
--set_member: (v: a) -> (s: set a) -> bool
set_member v (mkSet l) = true
...
combine: set -> set -> set
combine (mkSet b1) (mkSet b2) = (mkSet b1 b2)

set_union: (s1: set a) -> (s2: set a) -> set a
set_union (mkSet l1) (mkSet l2) = combine (mkSet l1 l2)
-------------------------------------
...
intersect nil _ = nil
intersect (h::t) l2 = h l2

-- Return the intersection of s1 and s2
-- set_intersection: (eq a) => list a -> list a -> list a
set_intersection (mkSet l1) (mkSet l2) = intersect (mkSet l1 l2)
...
list_difference (h::t1) (h::t2) = t1 t2
list_difference (h1::t) (h2::t) = h1 h2
...
-- Return true if p is true for some v in s, otherwise else false
-- set_exists: (p: a -> bool) -> (s: set a) -> bool
set_exists p (mkSet l) = ite (option v s)
                         true
                         (set_exists p s)
...
set_witness p (mkSet l) = ite (set_exists p s)
                          (some v)
                          none
...
element: list -> list -> pair
element la lb -> (mkPair la lb) (mkPair lb la)

set_product (mkSet la) (mkSet lb) = element
...
set_toString nil = ""
set_toString (v::nil) = (toString v)
set_toString (v::l) = (toString v) ++ ", " ++ (set_toString l)

instance (Serialize a) => Serialize (set a) where
  toString l = "{" ++ (set_toString l) ++ "}"
-------------------------------------
